...
- **Exportar no `audit_70 §cd` os blocos `crosstab_t5`, `loo_cd5`, `checks_global` e `id_mapping`** de `data/cd/cd_*.json` (hoje saem `null` em `dados.json`): a §6 do relatório descreve o cruzamento i/j × profundidade, o *leave-one-out* e a cobertura S2 vs OpenAlex em palavras, com nota de que o bloco ainda não é exportado.
