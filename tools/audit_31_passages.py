...
def is_bib_line(w):
    ...
    return (
        re.search(rf"{SUR},?\s*W", w)
        and len(re.findall(r"\b(19|20)\d{2}\b", w)) >= 2
        and re.search(
            r"(doi|Sustain|Transport|pp\.|vol\.|\d+\s*[-–]\s*\d+)", w, re.IGNORECASE
        )
    )
...
def by_name(t):
    ...
    for m in re.finditer(SUR, t):
        ...
        if is_bib_line(w) and SUR + " et al" not in w[:250]:
            continue
        out.append(w)
    return out
...
def by_number(t):
    m = re.search(
        r"(?im)^\s*(references|bibliography|refer[eê]ncias|literature cited)\s*$", t
    )
    ...
    for pat in (
        ...
        mm = re.search(pat, tail, re.DOTALL)
        if mm:
            num = mm.group(1)
            break
    if not num:
...
for key, r in auditlib.iter_records(master):
    ...
    for x in ps:
        ...
        if k not in seen:
            seen.add(k)
            ded.append(x)
    r["passages_auto"] = ded[:4]
    r["passages_how"] = how
    if ded:
        stats["com_passagem"] += 1
    elif SUR in t:
        r["citation_status_auto"] = "bibliography_only"
        stats["so_bibliografia"] += 1
    else:
        r["citation_status_auto"] = "not_found"
        stats["nao_achou"] += 1
...
novos = [
    r
    for k, r in auditlib.iter_records(master)
