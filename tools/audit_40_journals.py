...
def source_de(doi):
    ...
    return {
        ...
        "issn_l": loc.get("issn_l"),
        "host": loc.get("host_organization_name"),
        "type": loc.get("type"),
...
def meta(sid):
    s = auditlib.jget(
        f"{sid.replace('https://openalex.org/', 'https://api.openalex.org/sources/')}?mailto={MAIL}"
    )
    if not s:
        return sid, None
    ...
    return sid, {
        "id": sid,
        "nome": s.get("display_name"),
        "issn_l": s.get("issn_l"),
        "issn": s.get("issn"),
        "editora": s.get("host_organization_name"),
        "pais": s.get("country_code"),
        "tipo": s.get("type"),
        "is_oa": s.get("is_oa"),
        "in_doaj": s.get("is_in_doaj"),
        "h_index": st.get("h_index"),
        "citedness_2a": st.get("2yr_mean_citedness"),
        "i10": st.get("i10_index"),
