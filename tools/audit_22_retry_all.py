"""Etapa 22: varredura ampla — tenta TODA localização conhecida de TODO registro com DOI
...
def routes(doi):
    ...
    if w:
        for L in w.get("locations") or []:
            ...
            if L.get("pdf_url"):
                urls.append((0 if repo else 2, L["pdf_url"]))
            if L.get("landing_page_url"):
                urls.append((1 if repo else 3, L["landing_page_url"]))
    ...
    up = auditlib.jget(
        f"https://api.unpaywall.org/v2/{urllib.parse.quote(doi)}?email={MAIL}"
    )
    for loc in (up or {}).get("oa_locations") or []:
        if loc.get("url_for_pdf"):
            urls.append(
                (0 if loc.get("host_type") == "repository" else 2, loc["url_for_pdf"])
            )
        elif loc.get("url"):
            urls.append((1 if loc.get("host_type") == "repository" else 3, loc["url"]))
    seen = set()
    out = []
    ...
    for u in routes(rec["doi"]):
        try:
            ...
            if body[:4] == b"%PDF" or "pdf" in ctype.lower():
                t = save_and_extract(body, rec["id"])
                if t:
                ...
                for href in re.findall(
                    ...
                    try:
                        b2, _c2, f2 = fetch(urllib.parse.urljoin(final, href))
                        if b2[:4] == b"%PDF":
                            t2 = save_and_extract(b2, rec["id"])
                            if t2:
                                return t2, f"pdf:{f2}"
                    except Exception:
                        pass
...
todo = [
    r
    for k, r in auditlib.iter_records(M)
...
with ThreadPoolExecutor(max_workers=6) as ex:
    ...
    for i, f in enumerate(as_completed(futs), 1):
        ...
        if t:
            ...
            ok += 1
        if i % 25 == 0:
            print(f"   ...{i}/{len(todo)} (recuperados: {ok})")
print(f"\nRECUPERADOS NESTA VARREDURA: {ok}")
auditlib.save_master(M)
import collections

c = collections.Counter(r["status"] for k, r in auditlib.iter_records(M))
print(f"inventario: {sum(len(b['citing']) for b in M['papers'].values())}")
