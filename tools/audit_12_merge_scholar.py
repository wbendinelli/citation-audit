    ...
    for s in scholar:
        ...
        for hn, r in have:
            ...
            if hn.startswith(s["n"][:60]) or s["n"].startswith(hn[:60]):
                hit = r; break
            if len(s["n"]) > 30 and SequenceMatcher(None, s["n"][:76], hn[:76]).ratio() > 0.90:
                hit = r; break
        if hit:
            matched += 1
...
for k, r in report.items():
    print(f"{k:>9}{r['scholar']:>9}{r['junk']:>7}{r['casados']:>9}{r['novos']:>7}{r['antes']:>7}{r['depois']:>8}")
tot = lambda f: sum(r[f] for r in report.values())
print(f"{'TOTAL':>9}{tot('scholar'):>9}{tot('junk'):>7}{tot('casados'):>9}{tot('novos'):>7}{tot('antes'):>7}{tot('depois'):>8}")
