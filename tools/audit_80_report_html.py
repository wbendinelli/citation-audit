...
PT = {
    "bibliography_only": "só na bibliografia",
...
SCHOLAR = {
    k: sum(1 for l in (auditlib.DATA / "scholar" / f"{k}.txt").open() if l.strip())
...
def venue_norm(v):
    ...
    for a, b in fix.items():
        if v.lower().startswith(a.lower()[:40]):
            return b
    return v
...
def funil(key):
    ...
    return [
        ("Google Scholar reporta", SCHOLAR[key], None),
...
def render_funil(key):
    ...
    for i, (rot, val, delta) in enumerate(f):
        ...
        li.append(
            f"<li class='{cls_}'><span class='rot'>{rot}</span>"
...
def render_revistas(key):
    ...
    for r in master["papers"][key]["citing"]:
        d = r.get("doi") or ""
        if not d or d.split("/")[0] not in GRANDES or _livro(d):
            continue
        c = CL.get(d.lower())
        if c:
            rows.append((venue_norm(r.get("venue")), c))
    ...
    for v, cs in sorted(agg.items(), key=lambda x: (-len(x[1]), x[0])):
        ...
        out.append(
            f"<tr><td class='v'>{esc(v)}</td><td class='n'>{len(cs)}</td>"
...
def barras_q(key):
    c = collections.Counter(quart(r) for r in master["papers"][key]["citing"])
    segs = "".join(
        f"<span class='qseg {QCLS[x]}' style='flex:{c[x]}' title='{x}: {c[x]}'>{c[x]}</span>"
        for x in QORD
        if c.get(x)
    )
    leg = "".join(
        f"<span class='lg'><i class='sw {QCLS[x]}'></i>{x} <b>{c[x]}</b></span>"
        for x in QORD
        if c.get(x)
    )
...
def matriz_q():
    ...
    for x in QORD:
        if not tab[x]:
            continue
        tds = "".join(
            f"<td class='n {'hi' if tab[x][r] else 'off'}'>{tab[x][r] or '·'}</td>"
            for r in ROLES_ORD
        )
...
QBARS = "".join(
    f"<div class='fcol'><h3>{'Aviação' if k == 'airline' else 'Grãos'}</h3>{barras_q(k)}</div>"
    for k in ("airline", "grains")
)
QMATRIZ = matriz_q()
...
GHOST_Q1 = _ghost_q["Q1"]
GHOST_TOT = sum(_ghost_q.values())
...
if "--check" in sys.argv[1:]:
    ...
    if atual != HTML:
        ...
        print(
            f"DRIFT: report/index.html gerado difere do commitado "
            f"({nbytes} bytes gerados vs {atual_bytes} commitados)"
        )
        sys.exit(1)
    print(
        f"ok: report/index.html idêntico ao gerado ({nbytes} bytes | {TOT} citacoes | {NCL} classificadas)"
    )
else:
