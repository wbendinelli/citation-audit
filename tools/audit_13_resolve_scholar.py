...
def resolve(rec):
    ...
    # Crossref
    d = auditlib.jget(
        f"https://api.crossref.org/works?query.bibliographic={q}&rows=3&mailto={MAIL}"
    )
    ...
    # OpenAlex
    d = auditlib.jget(
        f"https://api.openalex.org/works?search={q}&per-page=3&mailto={MAIL}"
    )
