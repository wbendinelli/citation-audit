...
def enrich(rec):
    ...
    doi = rec.get("doi")
    if doi and (
        not rec.get("oa_status") or not rec.get("title") or not rec.get("venue")
    ):
        w = auditlib.jget(
        ...
        if w:
            ...
            rec.setdefault("title", w.get("title"))
            rec["title"] = rec.get("title") or w.get("title")
            ...
            rec["landing"] = rec.get("landing") or (
    ...
    if doi and not rec.get("pdf"):
        ...
        if b.get("url_for_pdf"):
            rec["pdf"] = b["url_for_pdf"]
        elif b.get("url"):
            rec["oa_landing"] = b["url"]
        if rec.get("is_oa") is None:
            rec["is_oa"] = bool(up and up.get("is_oa"))
    return rec
...
def triage(rec):
    ...
    if not rec.get("doi"):
        return "sem_doi"
    if rec.get("pdf") or rec.get("arxiv") or rec.get("pmcid"):
        return "oa_baixavel"
    if rec.get("is_oa") or rec.get("oa_landing"):
        return "oa_sem_pdf_direto"
    return "fechado"
