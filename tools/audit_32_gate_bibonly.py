...
for key, r in auditlib.iter_records(master):
    if r["status"] in TERMINAIS:
        continue
    if r.get("citation_status_auto") != "bibliography_only":
        continue
    p = r.get("text_path")
    ...
    if eh_pagina_de_rosto(t):
        ...
        r["nota_integridade"] = (
            "página de rosto do publisher: lista de referências sem o corpo"
        )
...
for key, r in auditlib.iter_records(master):
    ...
    if not c or c.get("role") != "bibliography_only":
        continue
    p = r.get("text_path")
    if not p or not (auditlib.ROOT / p).exists():
        revistas.append((r["id"], "sem arquivo para reconferir"))
        continue
    t = (auditlib.ROOT / p).read_text(encoding="utf-8", errors="ignore")
    if eh_pagina_de_rosto(t):
        revistas.append((r["id"], "era página de rosto"))
